...
(function () {
    ...
    if (location.pathname.match(/\/tutorial\//)) {
        ...
        if (j$("span[class=ttl_small]:contains(兵士の能力)").length != 0) {
            j$(document.body).append("<div id=AjaxTempDOM>");
            j$("#AjaxTempDOM").hide();
            j$("#AjaxTempDOM").load("http://" + HOST + "/facility/select_facility.php?x=0&y=1" + " #gray02Wrapper", function () {
                if (j$("p[class=sub] a[href*=build.php]:eq(0)").attr("href").match(/village_id=(\d+)/)) {
                    var b = RegExp.$1;
                    var c = "http://" + HOST + "/facility/build.php?id=209&x=0&y=1&village_id=" + b + "#ptop";
                    var d = "http://" + HOST + "/facility/build.php?id=215&x=3&y=5&village_id=" + b + "#ptop";
                    j$.get(c, function () {
                        j$.get(d, function () {
                            var a = {};
                            a['tuto_attack'] = "15";
                            a['tuto_defense_spear'] = "10";
    ...
    if (location.pathname == "/busyodas/busyodas.php") {
        ...
        j$("#AutoBushodasControls").append("<div id=notice_msg>※自動破棄を有効にしても<span class=Rarity_SR>SR</span>, <span class=Rarity_R>R</span>及び優良<span class=Rarity_UC>UC</span>(コスト3/劉備/孫権/諸葛亮/徐庶)は破棄対象外</div>");
        j$("#AutoBushodasControls").append("<div id=first_line_wrap><div id=delete_enabler title=R以上及び優良UC(コスト3/劉備/孫権/諸葛亮/徐庶)を除外><input type=checkbox id=auto_delete>自動破棄を有効にする</div><div id=save_btn><input type=button id=save_settings value=設定保存></div><div id=draw_btn><input type=button id=AutoDrawBushodas value=自動ブショーダス></div></div>");
        j$("#AutoBushodasControls").append("<div class=except_list>破棄除外(隠しスキル付与素材)</div>");
        ...
        j$("#AutoBushodasControls").append("<li class=keep_skill title=槍兵突撃/槍兵強行スキルを除外><input type=checkbox id=except_spear>槍兵の猛撃</li>");
...
function BuildFacility(f) {
    ...
    j$("#AjaxTempDOM").load("http://" + HOST + "/facility/select_facility.php?x=" + f[0].x + "&y=" + f[0].y + " #gray02Wrapper", function () {
        var a;
        var b = 0;
        var c = f[0].lv;
        var d;
        var e;
        j$("p[class=sub] a[href*=build.php]:eq(0)").attr("href").match(/village_id=(\d+)/);
        a = RegExp.$1;
        ...
        BuildTimeStr = j$("table:has(img[src*=facility_" + f[0].id + ".png]) tr:has(th:contains(所要時間)) td").html().trim();
        ...
        d = parseInt(RegExp.$1, 10) * 60 * 60 * 1000 + parseInt(RegExp.$2, 10) * 60 * 1000 + parseInt(RegExp.$3, 10) * 1000;
        if (b > 0) {
            e = "http://" + HOST + "/facility/build.php?x=" + f[0].x + "&y=" + f[0].y + "&village_id=" + a + "#ptop"
        } else {
            e = "http://" + HOST + "/facility/build.php?id=" + f[0].id + "&x=" + f[0].x + "&y=" + f[0].y + "&village_id=" + a + "#ptop"
        }
        f.shift();
        if (b >= c) {
            BuildFacility(f)
...
function AutoBushodas(n, o, p) {
    ...
    if (q == 0) {
        if (p != 0) {
            ...
            s['ssid'] = r;
            s['btn_send'] = "破棄";
            j$.post("http://" + HOST + "/card/allcard_delete.php", s, function () {
    ...
    var s = {};
    var t = Math.floor(Math.random() * 200 + 500);
    s['got_type'] = "0";
    s['send'] = "send";
    s['ssid'] = r;
    s['del_card_id'] = p;
    j$("#AjaxTempDOM").load("http://" + HOST + "/busyodas/busyodas.php #gray02Wrapper", s, function () {
        ...
        if (GM_getValue(KEY + "AutoDelete", false) == true) {
            j = a;
            h = " を自動削除しました!";
            if (c == "R" || c == "SR" || e >= 3.0 || b == "1007" || b == "1009" || b == "1014" || b == "3008") {
                j = "0";
                h = " が当たりました!"
            }
            if ((GM_getValue(KEY + "Keep_Hero", false) && g == "騎兵強行") || (GM_getValue(KEY + "Keep_Spear", false) && g == "槍兵突撃") || (GM_getValue(KEY + "Keep_Spear", false) && g == "槍兵強行") || (GM_getValue(KEY + "Keep_Cavalry", false) && g == "騎兵突撃") || (GM_getValue(KEY + "Keep_Cavalry", false) && g == "騎兵の強撃") || (GM_getValue(KEY + "Keep_Bow", false) && g == "弓兵突撃") || (GM_getValue(KEY + "Keep_Bow", false) && g == "弓兵の強撃") || (GM_getValue(KEY + "Keep_Ram", false) && g == "兵器の強撃") || (GM_getValue(KEY + "Keep_Courage", false) && g == "鉄壁") || (GM_getValue(KEY + "Keep_Thousand_Miles", false) && g == "急速援護") || (GM_getValue(KEY + "Keep_Intelligence", false) && f >= 15.0) || (GM_getValue(KEY + "Keep_Cost_C", false) && e >= 2.5) || (GM_getValue(KEY + "Keep_Cost_UC", false) && e >= 2.5 && c == "UC")) {
                j = "0";
                h = " が当たりました!"
            }
            var k = GM_getValue(KEY + "White_Lists", "").trim().split(",");
        ...
        j$("#CardInfo").html("<span id=card_rarity>" + c + "</span>  " + d + " (No." + b + ")<span id=result_msg>" + h + "</span>");
        if (c == "C") {
            j$("#card_rarity").css({
                "color": "black",
                "font-weight": "bold"
